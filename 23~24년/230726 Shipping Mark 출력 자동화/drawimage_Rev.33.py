...
# 자투리 파일이 역순으로 삽입됨
    # 두 번째 페이지를 별도의 리스트에 저장하고,
    # 모든 두 번째 페이지가 모여진 후에
    # 이를 역순으로 정렬하여 second_page_pool에 추가
...
def main():
    ...
    second_page_list = []  # 두 번째 페이지를 저장할 리스트 생성
    second_page_numbers = []  # 두 번째 페이지에 적용될 문서 번호를 저장할 리스트 생성
    ...
    while remaining_documents > 0:
        ...
        for selected_pdf_file in selected_pdf_files:
            ...
            if total_pages == 2:
                ...
                # 두 번째 페이지에 적용될 문서 번호를 리스트에 추가
                second_page_numbers.append(second_page_document_number)
            ...
            if total_pages == 2:
                ...
                second_page_list.append(second_page_with_watermark.pages[0])  # 리스트에 두 번째 페이지 추가
    ...
    # 모든 두 번째 페이지에 문서 번호를 적용
    second_page_pool = PdfWriter()
    for i, page in enumerate(second_page_list):
