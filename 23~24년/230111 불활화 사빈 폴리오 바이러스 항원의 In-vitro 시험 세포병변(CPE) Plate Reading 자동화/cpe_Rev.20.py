...
def remove_duplicate_circles(circles, threshold=50):
    unique_circles = []
...
for filepath in glob.glob(os.path.join(current_input_folder, "*.jpg")):
    ...
    circles = cv2.HoughCircles(mask, cv2.HOUGH_GRADIENT, dp=1, minDist=40,
                               param1=30, param2=20, minRadius=100, maxRadius=180)
    ...
    if circles is not None:
        ...
        for i, (x, y, r) in enumerate(unique_circles):
            ...
            cv2.putText(img, str(i + 1), (x - 3, y - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 10)
