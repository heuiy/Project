...
# 2번째 페이지를 모은 pdf 가 전체 pdf 의 마지막 숫자로 배정되는 것이 아니라
# 각각 중간으로 배정됨
...
def main():
    ...
    # 두 번째 페이지의 워터마크 시작 페이지 번호
    second_page_start_number = total_documents - remaining_documents + 1
    ...
    while remaining_documents > 0:
        ...
        for selected_pdf_file in selected_pdf_files:
            ...
            # 두 번째 페이지가 있다면, 첫 번째 페이지 복사 수에서 1을 빼고 전체 문서 수에서 1을 뺌
            if total_pages == 2:
                first_page_copies -= 1
            ...
            if total_pages == 2:
                ...
                second_page_with_watermark = add_watermark(second_page_temp_pdf_file, type_choice_second_page,
                                                           second_page_start_number, total_documents)
                second_page_pool.add_page(second_page_with_watermark.pages[0])

                # 다음 두 번째 페이지의 워터마크 번호를 위해 1 증가
                second_page_start_number += 1
