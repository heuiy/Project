...
def main():
    ...
    second_pages = []  # 두 번째 페이지를 수집할 리스트
    ...
    # 모든 두 번째 페이지를 올바른 순서로 second_page_pool에 추가
    for second_page in reversed(second_pages):
        second_page_pool.add_page(second_page)

    # 두 번째 페이지가 있는 경우 파일로 저장
