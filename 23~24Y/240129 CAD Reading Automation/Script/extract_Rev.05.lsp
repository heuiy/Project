;; 문서 경로에 24개 dwg 저장됨
;; 그나마 이게 나음 번호가 순서대로 지정됨
;; pdf 변환은 실패했음

(defun c:ExtractMultipleRegionsAndExportPDF ()
 (setq coords '(
       ...
       (if ss
         (progn
           ...
           (setq pdfpath (strcat "D:\\#.Secure Work Folder\\BIG\\Project\\23~24Y\\240129 CAD Reading Automation\\PDF\\out\\pdf_output_"
             (itoa index) ".pdf"))
           (command "_-EXPORTPDF" "_all" "" pdfpath "_.Y")
...
​
(princ "\nExtractMultipleRegionsAndExportPDF 명령어를 사용하세요. (명령어: ExtractMultipleRegionsAndExportPDF)")
(princ)
​​