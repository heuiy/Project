...
def find_purple_circles(image):
    # HSV 색상 공간으로 변환
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    # 보라색의 HSV 범위를 정의 (이 값들은 실험을 통해 최적화해야 합니다)
    lower_purple = np.array([100, 50, 200])
    ...
    # 마스크에서 원을 감지
    circles = cv2.HoughCircles(mask, cv2.HOUGH_GRADIENT, dp=1, minDist=300,
                               param1=40, param2=20, minRadius=150, maxRadius=180)
    ...
    if circles is not None:
        ...
        for (x, y, r) in circles:
            ...
            _, circle_binary = cv2.threshold(circle_gray, 20, 255, cv2.THRESH_TOZERO)
...
for filepath in glob.glob(os.path.join(current_input_folder, "*.jpg")):
    ...
    purple_circles = find_purple_circles(img)

    for i, (x, y, r) in enumerate(purple_circles):  # unique_circles 대신 purple_circles 사용
        row = y // (img.shape[0] // 8)
