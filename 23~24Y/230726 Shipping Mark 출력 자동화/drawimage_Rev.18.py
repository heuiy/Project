...
def copy_pages(input_pdf_path, first_copies, second_pages_writer):
    ...
    if total_pages == 2:  # 두 번째 페이지가 있으면 첫 번째 페이지 복사 수를 하나 줄임
        first_copies -= 1
        second_pages_writer.addPage(reader.getPage(1))
...
def main():
    ...
    second_pages_writer = PdfFileWriter()  # To collect second pages
    ...
    while current_document_number <= total_documents:
        ...
        writer = copy_pages(selected_pdf_file, current_batch_copies, second_pages_writer)
        ...
        output_pdf_file = os.path.join("C:\\Users\\User\\Downloads\\PDF",
                                       f"modified_{timestamp}_{current_document_number}.pdf")
    ...
    # Add the collected second pages at the end
    output_pdf_file = os.path.join("C:\\Users\\User\\Downloads\\PDF",
                                   f"second_pages_{datetime.now().strftime('%Y%m%d%H%M')}.pdf")
    with open(output_pdf_file, "wb") as output_pdf:
        second_pages_writer.write(output_pdf)

    print("모든 문서 번호가 추가되었습니다. 두 번째 페이지들이 모아진 파일이 생성되었습니다.")
