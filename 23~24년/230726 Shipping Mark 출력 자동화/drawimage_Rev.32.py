...
def main():
    ...
    current_document_number = 1  # 첫 번째 페이지에 사용될 문서 번호
    remaining_documents = total_documents  # 남은 문서 수
    second_page_document_number = total_documents  # 두 번째 페이지에 사용될 문서 번호
    second_pages = []  # 두 번째 페이지들을 저장할 리스트
    ...
    while remaining_documents > 0:
        ...
        for selected_pdf_file in selected_pdf_files:
            ...
            # 두 번째 페이지가 있는 경우
            if total_pages == 2:
                first_page_copies -= 1  # 첫 번째 페이지 복사 수를 1 감소
                remaining_documents -= 1  # 두 번째 페이지를 위해 전체 문서 수에서 1을 빼줌

            # 복사할 페이지 수와 남은 문서 수를 비교
            ...
            temp_pdf_file = f"temp_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.pdf"
            ...
            output_pdf_file = f"{input_pdf_filename}_{timestamp}_{current_document_number}.pdf"
            ...
            # 두 번째 페이지가 있는 경우
            if total_pages == 2:
                second_page_temp_pdf_file = f"temp_second_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.pdf"
                ...
                # 두 번째 페이지를 second_pages 리스트에 추가
                second_pages.append(second_page_with_watermark.pages[0])

                # 두 번째 페이지에 사용될 문서 번호 감소
    ...
    # 두 번째 페이지들을 모두 합쳐서 하나의 PDF 파일로 저장
    if len(second_pages) > 0:
        second_page_pool = PdfWriter()
        for page in second_pages:
            second_page_pool.add_page(page)

        output_pdf_file_second_page = f"all_second_pages_{timestamp}.pdf"
