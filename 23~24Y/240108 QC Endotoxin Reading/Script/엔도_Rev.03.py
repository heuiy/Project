...
def detect_blue_semicircles(image_path, output_path, folder_name):
    ...
    top_height = height // 3  # 상단 1/3 높이 계산
    img_top_third = img[:top_height, :]  # 이미지 상단 1/3 부분 잘라냄
    ...
    # Hough 변환 파라미터 조정
    circles = cv2.HoughCircles(mask_blue, cv2.HOUGH_GRADIENT, dp=1, minDist=20,
                               param1=50, param2=30, minRadius=10, maxRadius=30)

    # 파란색 반원의 수를 카운트합니다.
    blue_semicircle_count = 0
    detected = False  # 반원이 검출되었는지를 나타내는 플래그
    ...
    if not detected:
        cv2.putText(img, "No blue semicircles detected", (10, top_height + 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

    # 파란색 반원 개수를 원본 이미지에 표시합니다.
    cv2.putText(img, f"Blue Semicircles: {blue_semicircle_count if detected else 'None'}",
                (10, top_height + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)

    # 원본 이미지에 폴더명 삽입
    cv2.putText(img, folder_name, (img.shape[1] - 400, img.shape[0] - 20),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

    # 원본 이미지 크기에 상단 1/3에 표시한 결과를 저장합니다.
    cv2.imwrite(output_path, img)

    return blue_semicircle_count
