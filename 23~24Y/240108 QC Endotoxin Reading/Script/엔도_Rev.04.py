# 배치 번호 삽입
# 파란색 원 형태를 전혀 못 찾고 있음
# detect_blue_semicircles 반원을 찾으려 하니까 잘 안됨
...
def detect_blue_semicircles(image_path, output_path, folder_name):
    ...
    cv2.putText(img, f"Blue Semicircles: {blue_semicircle_count if detected else 'None'}",
                (10, top_height - 280), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 255, 255), 3)

    # 원본 이미지에 폴더명 삽입
    cv2.putText(img, folder_name, (img.shape[1] - 300, img.shape[0] - 50),
