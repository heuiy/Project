...
# 페이지 번호는 순서대로 삽입되는데, 자투리 pdf 가 역순으로 추가됨
...
def main():
    ...
    temp_second_page_pool = PdfWriter()  # 임시로 두 번째 페이지를 저장할 PDF Writer 객체
    ...
    while remaining_documents > 0:
        ...
        for selected_pdf_file in selected_pdf_files:
            ...
            if total_pages == 2:
                ...
                # 두 번째 페이지를 임시 PDF Writer 객체에 추가
                temp_second_page_pool.add_page(second_page_with_watermark.pages[0])
    ...
    # 임시로 저장한 두 번째 페이지를 역순으로 second_page_pool에 추가
    for page in reversed(temp_second_page_pool.pages):
