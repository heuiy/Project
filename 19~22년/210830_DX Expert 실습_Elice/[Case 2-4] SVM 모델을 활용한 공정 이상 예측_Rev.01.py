...
# # [Case2-4] 반도체 공정 데이터를 활용한 공정 이상 예측_Rev.01
...
# 
# 3. **SVM 모델 정의 :** SVM 모델 구현
# 
# 4. **하이퍼 파라미터 설정 및 컴파일 :** 올바른 하이퍼 파라미터 설정
# 
# 5. **모델 학습 수행:** <span style="color:red"> SVM 모델 </span>을 통한 학습 수행, 평가 및 예측 수행
# 
...
# 
# |Run|Epochs|Scaler|criterion|frac|max_iter|lit|min_leaf|criterion|n_trees|max_depth|n_batches|epoch|변수|lr|MSE|MAE|
# |-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|
# |01|-|Standard||0.8|100000||||||||||0.903226|0.205882|
# |02|-|MinMax||0.8|100000||||||||||0.858757|0.210526|
# |03|-|MinMax||0.9|100000||||||||||0.902878|0.228571|
# 
# class 0 / class 1의 f1-score가 각각 0.94 / 0.26 이상이면 100점
# 
...
plt.boxplot(normed_test_data[6], manage_ticks=False) # 데이터, 소눈금 표시 여부
...
# ---
# 
# - 최적의 결정 경계(Decision Boundary)를 결정하는 기준 선을 정의하는 모델.
# - 결정 경계와 가장 가까이에 있는 데이터 포인트를 기준으로 결정 경계를 형성하고 이 기준선에 여유(Margin)을 두어 이 여유를 최대화하는 결정 경계를 찾음.
# - 딥러닝 기술 등장 이전까지 가장 인기있는 분류 알고리즘.
# 
#  ![05_svm](02_04_file/05_svm.png)
...
linear_svm = LinearSVC(max_iter=100000)
...
linear_svm.fit(normed_train_data, train_labels)
...
# svm 모델이 예측.
prec_labels_svm = linear_svm.predict(normed_test_data)
...
# 평균 accuracy.
mean_acc = linear_svm.score(normed_test_data, test_labels)


# In[ ]:


print(classification_report(test_labels, prec_labels_svm))
...
plot_roc_curve(linear_svm, normed_test_data, test_labels)
...
linear_svm_optim = LinearSVC(max_iter=10000)


# In[ ]:


linear_svm_optim.fit(X_resampled, y_resampled)


# In[ ]:


prec_labels_svm = linear_svm_optim.predict(normed_test_data)
...
# 평균 accuracy.
mean_acc = linear_svm_optim.score(normed_test_data, test_labels)
...
plot_roc_curve(linear_svm_optim, normed_test_data, test_labels)
...
# ---
# 
# SVM 분류 모델을 구현하여 표준 스케일링 된 테스트 데이터(**normed_test_data**)를 추론해보세요.
# 
...
# 
# `sklearn.metrics`의 `classification_report`을 통한 성능 측정으로 **class 0 / class 1의 f1-score**가 각각 **0.94 / 0.26** 이상이면 100점입니다.
# 
# (부분점수 있음)
# 
...
answer_df = pd.DataFrame(prec_labels_svm)
